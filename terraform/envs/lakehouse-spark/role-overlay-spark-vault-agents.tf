/*
 * role-overlay-spark-vault-agents.tf -- Phase 0.L.3
 *
 * Installs nexus-vault-agent on each of the 5 SPARK nodes (2 masters + 3 workers)
 * -- NOT the 3 ZooKeeper nodes (backplane-plaintext, no Vault footprint). Port of
 * the iceberg vault-agents overlay: sidecar vault-agent-lakehouse-spark-<host>.json.
 *
 * Cross-env coupling: reads the AppRole creds JSON sidecars (security env writes
 * them). ERROR (not WARN+skip) if absent.
 *
 * Selective ops: var.enable_spark_vault_agents (master) AND per-host
 *                var.enable_spark_<n>_vault_agent.
 */

locals {
  spark_vault_agent_specs = {
    "spark-master-1" = { vm_ip = "192.168.70.140", enabled = var.enable_spark_master_1_vault_agent }
    "spark-master-2" = { vm_ip = "192.168.70.153", enabled = var.enable_spark_master_2_vault_agent }
    "spark-worker-1" = { vm_ip = "192.168.70.145", enabled = var.enable_spark_worker_1_vault_agent }
    "spark-worker-2" = { vm_ip = "192.168.70.146", enabled = var.enable_spark_worker_2_vault_agent }
    "spark-worker-3" = { vm_ip = "192.168.70.154", enabled = var.enable_spark_worker_3_vault_agent }
  }

  spark_vault_agent_active = {
    for host, spec in local.spark_vault_agent_specs : host => spec
    if var.enable_spark_vault_agents && spec.enabled
  }

  spark_va_creds_dir_expanded = pathexpand(replace(var.vault_agent_spark_creds_dir, "$HOME", "~"))
  spark_va_ca_bundle_expanded = pathexpand(replace(var.vault_pki_ca_bundle_path, "$HOME", "~"))
}

resource "null_resource" "spark_vault_agent" {
  for_each = local.spark_vault_agent_active

  triggers = {
    creds_file_path    = "${local.spark_va_creds_dir_expanded}/vault-agent-lakehouse-spark-${each.key}.json"
    creds_file_hash    = filesha256("${local.spark_va_creds_dir_expanded}/vault-agent-lakehouse-spark-${each.key}.json")
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      $credsFile    = '${local.spark_va_creds_dir_expanded}/vault-agent-lakehouse-spark-${each.key}.json'
      $caBundlePath = '${local.spark_va_ca_bundle_expanded}'
      $sshUser      = '${var.lakehouse_node_user}'
      $bootTimeout  = ${var.lakehouse_cluster_timeout_minutes}

      if (-not (Test-Path $credsFile)) {
        throw "[spark-va $hostName] creds file $credsFile missing -- run nexus-infra-vmware/scripts/security.ps1 apply FIRST to provision the 5 spark AppRole sidecars."
      }
      $creds     = Get-Content $credsFile | ConvertFrom-Json
      $roleId    = $creds.role_id
      $secretId  = $creds.secret_id
      $vaultAddr = $creds.vault_addr
      if (-not $roleId -or -not $secretId) { throw "[spark-va $hostName] creds JSON missing role_id or secret_id" }
      if (-not (Test-Path $caBundlePath)) { throw "[spark-va $hostName] CA bundle $caBundlePath missing -- run security env apply first." }

      $sshOpts = @('-o','ConnectTimeout=10','-o','BatchMode=yes','-o','StrictHostKeyChecking=no')

      Write-Host "[spark-va $hostName] waiting for SSH + firstboot marker..."
      ...
      if (-not $booted) { throw "[spark-va $hostName] SSH + firstboot marker never ready after $bootTimeout min" }
      ...
      if ($probe -match "Vault v$vaultVersion" -and $probe -match '(?m)^active$') {
        Write-Host "[spark-va $hostName] already installed at v$vaultVersion + service active; skipping."
        exit 0
      }

      Write-Host "[spark-va $hostName] installing Vault Agent v$vaultVersion"
      $installScript = @"
set -euo pipefail
...
"@
      ...
      if ($LASTEXITCODE -ne 0) { Write-Host $installOut.Trim(); throw "[spark-va $hostName] vault binary install failed (rc=$LASTEXITCODE)" }
...
"@
        ...
        if ($LASTEXITCODE -ne 0) { Write-Host $stageOut.Trim(); throw "[spark-va $hostName] credential staging failed (rc=$LASTEXITCODE)" }
      } finally {
      ...
      $baseConfig = @"
# 00-base.hcl -- Phase 0.L.3 (Spark HA mTLS).
...
"@
      $unitFile = @"
[Unit]
Description=Nexus Vault Agent (Phase 0.L.3 -- Spark HA mTLS)
...
"@
      ...
      if ($LASTEXITCODE -ne 0) { Write-Host $finalOut.Trim(); throw "[spark-va $hostName] config/service setup failed (rc=$LASTEXITCODE)" }
      ...
      if (-not $serviceActive) {
        ...
        Write-Host $journal
        throw "[spark-va $hostName] nexus-vault-agent.service failed to reach active within 30s"
      }
      $tokenCheck = (ssh @sshOpts "$sshUser@$vmIp" "sudo test -s /var/run/nexus-vault-agent/token && echo TOKEN_PRESENT" 2>&1 | Out-String).Trim()
      if ($tokenCheck -notmatch 'TOKEN_PRESENT') { throw "[spark-va $hostName] AppRole login appears to have failed (token sink empty)" }
      Write-Host "[spark-va $hostName] nexus-vault-agent.service active; AppRole authenticated"
    PWSH
  }
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      Write-Host "[spark-va destroy] $${hostName}: stopping nexus-vault-agent + cleaning install-owned files"
