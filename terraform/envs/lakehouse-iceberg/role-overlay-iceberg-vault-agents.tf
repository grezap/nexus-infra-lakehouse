/*
 * role-overlay-iceberg-vault-agents.tf -- Phase 0.L.2
 *
 * Installs nexus-vault-agent on each of the 4 iceberg nodes (2 PG + 2 REST).
 * Port of the MinIO vault-agents overlay: sidecar filename
 * vault-agent-lakehouse-iceberg-<host>.json + the iceberg node set.
 *
 * Cross-env coupling: reads the AppRole creds JSON sidecars (security env writes
 * them). ERROR (not WARN+skip) if absent.
 *
 * Selective ops: var.enable_iceberg_vault_agents (master) AND per-host
 *                var.enable_iceberg_<n>_vault_agent.
 */

locals {
  iceberg_vault_agent_specs = {
    "iceberg-rest-1" = { vm_ip = "192.168.70.147", enabled = var.enable_iceberg_rest_1_vault_agent }
    "iceberg-rest-2" = { vm_ip = "192.168.70.148", enabled = var.enable_iceberg_rest_2_vault_agent }
    "iceberg-pg-1"   = { vm_ip = "192.168.70.149", enabled = var.enable_iceberg_pg_1_vault_agent }
    "iceberg-pg-2"   = { vm_ip = "192.168.70.150", enabled = var.enable_iceberg_pg_2_vault_agent }
  }

  iceberg_vault_agent_active = {
    for host, spec in local.iceberg_vault_agent_specs : host => spec
    if var.enable_iceberg_vault_agents && spec.enabled
  }

  iceberg_va_creds_dir_expanded = pathexpand(replace(var.vault_agent_iceberg_creds_dir, "$HOME", "~"))
  iceberg_va_ca_bundle_expanded = pathexpand(replace(var.vault_pki_ca_bundle_path, "$HOME", "~"))
}

resource "null_resource" "iceberg_vault_agent" {
  for_each = local.iceberg_vault_agent_active

  triggers = {
    creds_file_path      = "${local.iceberg_va_creds_dir_expanded}/vault-agent-lakehouse-iceberg-${each.key}.json"
    creds_file_hash      = filesha256("${local.iceberg_va_creds_dir_expanded}/vault-agent-lakehouse-iceberg-${each.key}.json")
    nftables_id          = length(null_resource.iceberg_nftables_backplane) > 0 ? null_resource.iceberg_nftables_backplane[0].id : "disabled"
    vault_version        = var.vault_agent_version
    iceberg_va_overlay_v = "1"

    destroy_vm_ip    = each.value.vm_ip
    destroy_ssh_user = var.lakehouse_node_user
  }

  depends_on = [null_resource.iceberg_nftables_backplane]
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      $credsFile    = '${local.iceberg_va_creds_dir_expanded}/vault-agent-lakehouse-iceberg-${each.key}.json'
      $caBundlePath = '${local.iceberg_va_ca_bundle_expanded}'
      $sshUser      = '${var.lakehouse_node_user}'
      $bootTimeout  = ${var.lakehouse_cluster_timeout_minutes}

      if (-not (Test-Path $credsFile)) {
        throw "[iceberg-va $hostName] creds file $credsFile missing -- run nexus-infra-vmware/scripts/security.ps1 apply FIRST to provision the 4 iceberg AppRole sidecars."
      }
      $creds     = Get-Content $credsFile | ConvertFrom-Json
      $roleId    = $creds.role_id
      $secretId  = $creds.secret_id
      $vaultAddr = $creds.vault_addr
      if (-not $roleId -or -not $secretId) { throw "[iceberg-va $hostName] creds JSON missing role_id or secret_id" }
      if (-not (Test-Path $caBundlePath)) { throw "[iceberg-va $hostName] CA bundle $caBundlePath missing -- run security env apply first." }

      $sshOpts = @('-o','ConnectTimeout=10','-o','BatchMode=yes','-o','StrictHostKeyChecking=no')

      Write-Host "[iceberg-va $hostName] waiting for SSH + firstboot marker..."
      $bootDeadline = (Get-Date).AddMinutes($bootTimeout)
      $booted = $false
      while ((Get-Date) -lt $bootDeadline) {
        $probe = (ssh @sshOpts "$sshUser@$vmIp" "test -f /var/lib/lakehouse-node-firstboot-done && echo READY" 2>&1 | Out-String).Trim()
        if ($probe -match 'READY') { $booted = $true; break }
        Start-Sleep -Seconds 15
      }
      if (-not $booted) { throw "[iceberg-va $hostName] SSH + firstboot marker never ready after $bootTimeout min" }
      ...
      if ($probe -match "Vault v$vaultVersion" -and $probe -match '(?m)^active$') {
        Write-Host "[iceberg-va $hostName] already installed at v$vaultVersion + service active; skipping."
        exit 0
      }

      Write-Host "[iceberg-va $hostName] installing Vault Agent v$vaultVersion"
      $installScript = @"
set -euo pipefail
...
"@
      ...
      if ($LASTEXITCODE -ne 0) { Write-Host $installOut.Trim(); throw "[iceberg-va $hostName] vault binary install failed (rc=$LASTEXITCODE)" }
...
"@
        ...
        if ($LASTEXITCODE -ne 0) { Write-Host $stageOut.Trim(); throw "[iceberg-va $hostName] credential staging failed (rc=$LASTEXITCODE)" }
      } finally {
      ...
      $baseConfig = @"
# 00-base.hcl -- Phase 0.L.2 (Iceberg catalog mTLS).
...
"@
      $unitFile = @"
[Unit]
Description=Nexus Vault Agent (Phase 0.L.2 -- Iceberg catalog mTLS)
...
"@
      ...
      if ($LASTEXITCODE -ne 0) { Write-Host $finalOut.Trim(); throw "[iceberg-va $hostName] config/service setup failed (rc=$LASTEXITCODE)" }
      ...
      if (-not $serviceActive) {
        ...
        Write-Host $journal
        throw "[iceberg-va $hostName] nexus-vault-agent.service failed to reach active within 30s"
      }
      $tokenCheck = (ssh @sshOpts "$sshUser@$vmIp" "sudo test -s /var/run/nexus-vault-agent/token && echo TOKEN_PRESENT" 2>&1 | Out-String).Trim()
      if ($tokenCheck -notmatch 'TOKEN_PRESENT') { throw "[iceberg-va $hostName] AppRole login appears to have failed (token sink empty)" }
      Write-Host "[iceberg-va $hostName] nexus-vault-agent.service active; AppRole authenticated"
    PWSH
  }
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      Write-Host "[iceberg-va destroy] $${hostName}: stopping nexus-vault-agent + cleaning install-owned files"
