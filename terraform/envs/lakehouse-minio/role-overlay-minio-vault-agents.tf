/*
 * role-overlay-minio-vault-agents.tf -- Phase 0.L.1
 *
 * Installs nexus-vault-agent on each of the 4 MinIO nodes. Port of the
 * StarRocks vault-agents overlay with MinIO specifics: sidecar filename
 * vault-agent-lakehouse-minio-<host>.json + the MinIO node set.
 *
 * Cross-env coupling: reads the AppRole creds JSON sidecars (security env writes
 * them). ERROR (not WARN+skip) if absent.
 *
 * Selective ops: var.enable_minio_vault_agents (master) AND per-host
 *                var.enable_minio_<n>_vault_agent.
 */

locals {
  minio_vault_agent_specs = {
    "minio-1" = { vm_ip = "192.168.70.141", enabled = var.enable_minio_1_vault_agent }
    "minio-2" = { vm_ip = "192.168.70.142", enabled = var.enable_minio_2_vault_agent }
    "minio-3" = { vm_ip = "192.168.70.143", enabled = var.enable_minio_3_vault_agent }
    "minio-4" = { vm_ip = "192.168.70.144", enabled = var.enable_minio_4_vault_agent }
  }

  minio_vault_agent_active = {
    for host, spec in local.minio_vault_agent_specs : host => spec
    if var.enable_minio_vault_agents && spec.enabled
  }

  minio_va_creds_dir_expanded = pathexpand(replace(var.vault_agent_minio_creds_dir, "$HOME", "~"))
  minio_va_ca_bundle_expanded = pathexpand(replace(var.vault_pki_ca_bundle_path, "$HOME", "~"))
}

resource "null_resource" "minio_vault_agent" {
  ...
  depends_on = [null_resource.minio_nftables_backplane]
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      $credsFile    = '${local.minio_va_creds_dir_expanded}/vault-agent-lakehouse-minio-${each.key}.json'
      $caBundlePath = '${local.minio_va_ca_bundle_expanded}'
      $sshUser      = '${var.lakehouse_node_user}'
      $bootTimeout  = ${var.lakehouse_cluster_timeout_minutes}

      if (-not (Test-Path $credsFile)) {
        throw "[minio-va $hostName] creds file $credsFile missing -- run nexus-infra-vmware/scripts/security.ps1 apply FIRST to provision the 4 MinIO AppRole sidecars."
      }
      $creds     = Get-Content $credsFile | ConvertFrom-Json
      $roleId    = $creds.role_id
      $secretId  = $creds.secret_id
      $vaultAddr = $creds.vault_addr
      if (-not $roleId -or -not $secretId) { throw "[minio-va $hostName] creds JSON missing role_id or secret_id" }
      if (-not (Test-Path $caBundlePath)) { throw "[minio-va $hostName] CA bundle $caBundlePath missing -- run security env apply first." }

      $sshOpts = @('-o','ConnectTimeout=10','-o','BatchMode=yes','-o','StrictHostKeyChecking=no')

      Write-Host "[minio-va $hostName] waiting for SSH + firstboot marker..."
      ...
      if (-not $booted) { throw "[minio-va $hostName] SSH + firstboot marker never ready after $bootTimeout min" }
      ...
      if ($probe -match "Vault v$vaultVersion" -and $probe -match '(?m)^active$') {
        Write-Host "[minio-va $hostName] already installed at v$vaultVersion + service active; skipping."
        exit 0
      }

      Write-Host "[minio-va $hostName] installing Vault Agent v$vaultVersion"
      $installScript = @"
set -euo pipefail
...
"@
      ...
      if ($LASTEXITCODE -ne 0) { Write-Host $installOut.Trim(); throw "[minio-va $hostName] vault binary install failed (rc=$LASTEXITCODE)" }
...
"@
        ...
        if ($LASTEXITCODE -ne 0) { Write-Host $stageOut.Trim(); throw "[minio-va $hostName] credential staging failed (rc=$LASTEXITCODE)" }
      } finally {
      ...
      $baseConfig = @"
# 00-base.hcl -- Phase 0.L.1 (MinIO mTLS).
...
"@
      $unitFile = @"
[Unit]
Description=Nexus Vault Agent (Phase 0.L.1 -- MinIO mTLS)
...
"@
      ...
      if ($LASTEXITCODE -ne 0) { Write-Host $finalOut.Trim(); throw "[minio-va $hostName] config/service setup failed (rc=$LASTEXITCODE)" }
      ...
      if (-not $serviceActive) {
        ...
        Write-Host $journal
        throw "[minio-va $hostName] nexus-vault-agent.service failed to reach active within 30s"
      }
      $tokenCheck = (ssh @sshOpts "$sshUser@$vmIp" "sudo test -s /var/run/nexus-vault-agent/token && echo TOKEN_PRESENT" 2>&1 | Out-String).Trim()
      if ($tokenCheck -notmatch 'TOKEN_PRESENT') { throw "[minio-va $hostName] AppRole login appears to have failed (token sink empty)" }
      Write-Host "[minio-va $hostName] nexus-vault-agent.service active; AppRole authenticated"
    PWSH
  }
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      Write-Host "[minio-va destroy] $${hostName}: stopping nexus-vault-agent + cleaning install-owned files"
